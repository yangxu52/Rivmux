...
- 解复用器将容器载荷交给视频或音频归一化器；归一化器只产出 codec 配置和 `EncodedSample`，不依赖 fMP4 事件。当前支持 AVC/HEVC length-prefixed NAL/Annex-B 与 AAC raw access unit/ADTS；未来容器只需构造相同的归一化输入。
